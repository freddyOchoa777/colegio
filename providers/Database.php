<?php
	...
	{	
		...
		// Metodos de la clase para hacer una Crud select con varios parametros where
		public function select2($strSql, $arrayData = array(), $fetchMode = PDO::FETCH_OBJ)
		{
			//prepara el query
			$query = $this->prepare($strSql);
			...
			//asigna parametros al query si llegan como un array
			/*foreach ($arrayData as $key => $value) {*/
				$query->bindParam(":nomUsu", $arrayData['nomUsu'], PDO::PARAM_STR, 30);
				$query->bindParam(":idRol", $arrayData['idRol'], PDO::PARAM_INT);
			/*}*/
			//valida si se ejecuta o no el query
			if(!$query->execute()) {
				echo "Error, la Consulta no se Realizó";
